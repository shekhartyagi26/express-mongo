...
Crawler.prototype._crawlUrl = function(url, referer, depth) {
    ...
    var self = this;
    let proxyUrl = process.env.PROXY || null;
    this._requestUrl({
        url: url,
        ...
        if (proxyUrl && !error) {
            if ("error" == response.body.substr(2, 5)) {
        ...
        var body = isTextContent ? self._getDecodedBody(response) : isTextContent;
        if (proxyUrl && !error) {
            body = isTextContent ? self._getDecodedBody(response) : response && response.body;
        }
...
Crawler.prototype._getAllUrls = function(defaultBaseUrl, body) {
    var domainName = this._getHostName(defaultBaseUrl);
    var self = this;
    ...
    var urls = _.chain(links)
        .map(function(link) {
            ...
            var linkDomainName = self._getHostName(link);
            if (domainName != null && domainName == linkDomainName) {
                return link;
            } else {
                return '';
            }
        ...
        .value();
    urls = _.without(urls, '');
    return _.union(urls, this._getJsUrl(body), JS_URL);
};
...
Crawler.prototype._getJsUrl = (response_data) => {
    let url_to_scrap = [];
...
Crawler.prototype._getHostName = (url) => {
